...
def loss(logits, labels, ignore_label=-1, cb=None, name='loss'):
    with tf.name_scope(name):
        ...
        if cb is not None:
            ...
            xe = tf.nn.softmax_cross_entropy_with_logits_v2(
...
def predict(logits, name='predict'):
    with tf.name_scope(name):
        ...
        preds = tf.cast(tf.argmax(probs, axis=-1), tf.int32, name="preds")
    return probs, preds
...
def n_enc_block(inputs, phase_train, n, k, name):
    ...
    with tf.variable_scope(name):
        for i in range(n):
            h = ops.conv2d(h, k, 3, stride=1, name='conv_{}'.format(i + 1))
            h = ops.batch_norm(h, phase_train, name='bn_{}'.format(i + 1))
            h = ops.relu(h, name='relu_{}'.format(i + 1))
        h, mask = ops.maxpool2d_with_argmax(h, name='maxpool_{}'.format(i + 1))
    return h, mask
...
def n_dec_block(inputs, mask, adj_k, phase_train, n, k, name):
    with tf.variable_scope(name):
        h = ops.maxunpool2d(inputs, mask, name='unpool')
        for i in range(n):
            if i == (n - 1) and adj_k:
                h = ops.conv2d(h, k / 2, 3, stride=1, name='conv_{}'.format(i + 1))
            else:
                h = ops.conv2d(h, k, 3, stride=1, name='conv_{}'.format(i + 1))
            h = ops.batch_norm(h, phase_train, name='bn_{}'.format(i + 1))
            h = ops.relu(h, name='relu_{}'.format(i + 1))
...
def dec_last_conv(inputs, phase_train, k, name):
    with tf.variable_scope(name):
        h = ops.conv2d(inputs, k, 1, name='conv')
    return h
